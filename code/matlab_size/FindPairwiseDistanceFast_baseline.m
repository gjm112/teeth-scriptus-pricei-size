%this uses GeodesicElasticClosedFast_NS_baseline.m to find the pairwise distances between
%each closed curve in a dataset 

%the output will be a n x n symmetric matrix of distances with 0s on the
%diagonal 

function d_mat = FindPairwiseDistanceFast_baseline(data)
...
for i=1:n
   for j=(i+1):n
       row_col = [i,j]
       d_mat(i,j) = GeodesicElasticClosedFast_NS_baseline(data(:,:,i),data(:,:,j));
   end
end
